...
Pairing is local-only: create its QR from the local Remote dialog, then either scan it or use **Copy one-time link** to transfer the compact pairing link privately to the browser that will be paired. Treat the copied link exactly like the QR: anyone who gets it before expiry can pair, so never put it in regular chat, email, or a public document. Each pairing secret is single-use and expires in two minutes. The remote browser creates a non-extractable P-256 key in IndexedDB; clearing that browser profile, using a different origin, or losing the key requires a new local pairing. If every paired device is lost, regain local access to create a new pairing session. Local access is never granted through the remote gateway.
